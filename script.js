document.addEventListener('DOMContentLoaded', () => {
        ...
        if (loader) {
            ...
            setTimeout(() => {
                if (curtain) {
                    ...
                    curtain.style.transition = 'transform 1s cubic-bezier(0.77, 0, 0.175, 1)';
                    ...
                    setTimeout(() => {
                        ...
                        if (heroBg) {
                            ...
                            heroBg.style.animation = 'heroReveal 2.5s cubic-bezier(0.77, 0, 0.175, 1) forwards';
                        }
                    }, 800);
                } else {
                    triggerHeroAnimations();
    ...
    document.addEventListener('mousemove', (e) => {
        ...
        // Create elegant stardust trail - further throttled for speed
        if (Math.random() > 0.95) { 
            createSparkle(mouseX, mouseY, false);
        }
    });
    ...
    function triggerHeroAnimations() {
        ...
        lineContents.forEach((el, index) => {
            ...
            el.style.opacity = '0';
            el.style.transition = `transform 1.4s cubic-bezier(0.16, 1, 0.3, 1) ${index * 0.15 + 0.2}s, opacity 1.4s ease ${index * 0.15 + 0.2}s`;
        ...
        fadeUps.forEach((el, index) => {
            ...
            el.style.transition = `opacity 1.2s ease ${index * 0.2 + 0.8}s, transform 1.2s cubic-bezier(0.16, 1, 0.3, 1) ${index * 0.2 + 0.8}s`;
    ...
    if (canvas) {
        ...
        const initParticles = () => {
            ...
            const glowCount = window.innerWidth < 768 ? 3 : 8;
    ...
    // 12. Dynamic Thoranam Generator
    const thoranamWrapper = document.getElementById('thoranamWrapper');
    if (thoranamWrapper) {
        let thoranamTimer;
        const generateThoranam = () => {
            thoranamWrapper.innerHTML = '<div class="thoranam-string"></div>';
            const screenWidth = window.innerWidth;
            const itemWidth = 36;
            const count = Math.ceil(screenWidth / itemWidth) + 2;

            const fragment = document.createDocumentFragment();
            for (let i = 0; i < count; i++) {
                const item = document.createElement('div');
                item.className = 'thoranam-item ' + (i % 3 === 0 ? 'marigold' : 'mango-leaf');
                fragment.appendChild(item);
            }
            thoranamWrapper.appendChild(fragment);
        };
        generateThoranam();

        window.addEventListener('resize', () => {
            clearTimeout(thoranamTimer);
            thoranamTimer = setTimeout(generateThoranam, 250);
        });
    }
});
